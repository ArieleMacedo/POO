...
export class Banco {
 ...
 constructor(){
  ...
  consultaPorNumero(numero: string): Conta {
  ...
  transferir(origemConta: string, destinoConta: string, valor: number): void {
  ...
  listarContasCliente(cpf: string): void {
  ...
  listarConta(numero: string): void {
    ...
    if (contaPrint.cliente) {
      console.log(`
        ...
        Data de Nascimento: ${contaPrint.cliente.dataNascimento.toLocaleDateString(
          "pt-BR"
        )}`);
    } else {
      console.log(
      `A conta número ${contaPrint.numero} não possui cliente associado.`);
    }
  }
  ...
  totalizarSaldoCliente(cpf: string): number {
    ...
    const saldoCliente = cliente.contas.reduce(
  ...
  alteraTitularidade(numero: string, cpf: string ): void {
  ...
  excluirConta(numero: string): void {
    ...
    const indice = this._contas.findIndex(
      (c) => c.numero === contaExiste.numero
    );
    this._contas.splice(indice, 1);
    contaExiste.cliente.contas = contaExiste.cliente.contas.filter(
      (c) => c.numero !== contaExiste.numero
    );
  }
  ...
  listarDados(contas: Conta[]): void {
    contas.forEach((c) => {
      console.log(`Número: ${c.numero} | Saldo: ${c.saldo}`);
    });
  }

  listarCliente(cliente: Cliente): void {
    console.log(`
        ID: ${cliente.id}
        Nome: ${cliente.nome}
        CPF: ${cliente.cpf}
  ...
  carregarDados(): void{
        let conta1: Conta = new Conta(1,"111-1", new Date(), 300);
        let conta2: Conta = new Conta(2, "222-2", new Date(),100);
        let conta3: Conta = new Conta(3,"333-3",new Date(), 0);
        let conta4: Conta = new Conta(4,"444-4", new Date(), 50);
        ...
        let cliente1: Cliente = new Cliente(3,"Ariele", '825', new Date(1979, 6, 29));
        let cliente2: Cliente = new Cliente(2,"Felipe", '999', new Date(2004, 4, 24));
